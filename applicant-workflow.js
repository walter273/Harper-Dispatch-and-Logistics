...
function createWorkflow({ env = process.env, getStore, persist, fetchImpl = fetch }) {
  ...
  function readiness(record, review, w) {
    ...
    const payment = record.fields.billing_method === 'percentage' ? Boolean(w.billingReference) : Boolean(account && s.operations.billingSubscriptions.some(b => b.companyId === w.companyId && b.userId === account.id && b.plan === record.fields.dispatch_package && b.truckCount === Number(record.fields.available_units) && b.status === 'active' && (b.provider !== 'square' || require('./square-billing').entitled(b))));
