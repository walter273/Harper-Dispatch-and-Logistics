...
function seed(file, count = 255) {
  ...
  const accounts = { users, companies: [{ id: 'alphaway', name: 'Alphaway Logistics', status: 'active' }], sessions: users.map(user => ({ userId: user.id, tokenHash: createHash('sha256').update(tokens[user.id]).digest('hex'), expiresAt: Date.now() + 3600000 })) };
  ...
  intakes.push({ id: 'legacy-carrier', type: 'carrier-onboarding', createdAt: Date.now(), fields: { legal_carrier_name: 'Summit Ridge Transport', primary_contact: 'Casey Morgan', business_email: 'casey@example.com', business_phone: '303-555-0145', mc_number: '1234567', dot_number: '123456', equipment_type: 'Dry Van', available_units: '2', preferred_lanes: 'Denver to Phoenix', dispatch_package: 'dispatch-standard', billing_method: 'weekly', operational_notes: 'Please review the insurance renewal before dispatch begins.' } });
