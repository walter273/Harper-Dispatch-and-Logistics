(() => {
 ...
 if(!form)return;
 let device=null, connection=null, epoch=0, active=false, sdkPromise=null;
 ...
 function cleanup(){connection?.disconnect();connection=null;device?.destroy();device=null;active=false;hangup.disabled=true;}
 form.addEventListener('submit',async event=>{
  ...
  try{await sdk();const data=await request('/api/voice/token',{phone:form.elements.phone.value});if(version!==epoch||!active)return;device=new Twilio.Device(data.token,{logLevel:1});device.on('error',()=>{status.textContent='Call connection failed. Check microphone permission and your connection.';cleanup();refresh();});connection=await device.connect({params:{CallIntent:data.callId}});if(version!==epoch||!active){cleanup();return;}connection.on('accept',()=>{status.textContent='Call connected.';});connection.on('disconnect',()=>{status.textContent='Call ended.';cleanup();refresh();});connection.on('error',()=>{status.textContent='The call could not connect.';cleanup();refresh();});status.textContent='Calling…';}
