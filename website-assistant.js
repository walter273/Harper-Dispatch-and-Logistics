...
function createWebsiteAssistant({ env = process.env, fetchImpl = fetch } = {}) {
  ...
  async function ask(question) {
    ...
    if (!prompt || prompt.length > 1000) {
      throw Object.assign(
        new Error("Enter a question under 1,000 characters."),
        { statusCode: 400 }
      );
    }
    if (!key)
      throw Object.assign(
        new Error("The website AI assistant is not configured yet."),
        { statusCode: 503 }
      );
    ...
    try {
      ...
      if (!response.ok)
        throw Object.assign(
          new Error("The website AI assistant is temporarily unavailable."),
          { statusCode: 502 }
        );
      ...
      if (!text)
        throw Object.assign(
          new Error("The website AI assistant returned an empty response."),
          { statusCode: 502 }
        );
