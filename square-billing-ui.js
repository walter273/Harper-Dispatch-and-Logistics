(() => {
  ...
  el('testPlan').onsubmit = event => {event.preventDefault(); const form=event.currentTarget; run(form.querySelector('button'),async()=>{ const plan=form.elements.plan.value;await request('./api/billing/checkout',{plan,truckCount:window.HarperDispatch.isDispatchPlan(plan)?Number(form.elements.trucks.value):1,billingMethod:'weekly',termsVersion:window.HarperDispatch.termsVersion});render(await request('./api/square/state'));});};
